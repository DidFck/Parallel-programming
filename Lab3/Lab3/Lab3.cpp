...
int main()
{
    Vector<double> v(10000); // Создаем вектор из 10000 элементов
    v.initialize_random(1.0, 100.0); // Инициализация случайными числами в диапазоне [1, 100]

    std::cout << "Min: " << v.find_min() << "\n";
    std::cout << "Max: " << v.find_max() << "\n";
    std::cout << "Mean: " << v.calculate_mean() << "\n";

    v.parallel_sum(4); // Сумма элементов с использованием 4 потоков

    std::cout << "Euclidean Norm: " << v.calculate_euclidean_norm() << "\n";
    std::cout << "Manhattan Norm: " << v.calculate_manhattan_norm() << "\n";
